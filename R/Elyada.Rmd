...
pdac@reductions$fitsne <- CreateDimReducObject(embeddings = embed, 
                                               ...
                                               assay = "SCT", 
                                               global = TRUE)
p3 <- DimPlot(pdac, reduction = "fitsne", group.by = "seurat_clusters", pt.size = 0.75) + 
      scale_color_paletteer_d("ggthemes::Classic_20") + 
...
```{r}
nkt_cells <- rownames(pdac@meta.data[pdac@meta.data$seurat_clusters %in% c(1, 3, 7), ])
p48 <- DimPlot(pdac, reduction = "tsne", cells.highlight = nkt_cells, cols.highlight = "navy", pt.size = 0.75) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       NoLegend()
p48 / p3
...
#### Recluster Tumor NK / T Cells

Let's use `SCISSORS` to recluster the NK / T cells that originated from tumor samples. 

```{r}
nkt_tumor <- ReclusterCells(subset(pdac, subset = condition == "PDAC"), 
                            which.clust = c(1, 3, 7), 
                            merge.clusters = TRUE, 
                            n.HVG = 4000, 
                            n.PC = 20, 
                            ...
                            redo.embedding = TRUE, 
                            random.seed = 629)
nkt_tumor_pc <- Embeddings(nkt_tumor, reduction = "pca")
```
...
# run Fit-SNE
affin_nkt_tumor = PerplexityBasedNN(nkt_tumor_pc, perplexity=30, random_state=629)
...
embed_nkt_tumor1 = tsne_nkt_tumor.optimize(n_iter=150, exaggeration=10, momentum=0.6)
embed_nkt_tumor2 = embed_nkt_tumor1.optimize(n_iter=750, exaggeration=1, momentum=0.4)
```

We pull the results into R, making sure to save the default Barnes-Hut t-SNE results in another reduction slot.

```{r}
embed_nkt_tumor <- as.matrix(py$embed_nkt_tumor2)
...
nkt_tumor@reductions$tsne <- CreateDimReducObject(embeddings = embed_nkt_tumor, 
                                                  ...
                                                  assay = "SCT", 
                                                  global = TRUE)
p49 <- DimPlot(nkt_tumor, reduction = "tsne", pt.size = 1.5) + 
       scale_color_manual(values = paletteer_d("ggsci::default_nejm")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       guides(color = guide_legend(nrow = 1, override.aes = list(size = 4)))
...
We identify the naive & memory CD4+ T cells in clusters 0 & 1 using IL7R, CCR7, and CD69. Due to technical aspects of 10X Genomics' single cell capture, CD4 is very lowly expressed & thus isn't much use as a marker gene. 

```{r}
p50 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "IL7R", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p51 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "CD69", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p52 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "CCR7", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
We identify CD8+ T cells in clusters 2 & 4 using CD8A, with cluster 2 expressing the CD8+ T cell exhaustion marker EOMES. In additin, we can see NK cells in cluster 5 through its expression of GNLY & NKG7. 

```{r}
p53 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "CD8A", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p54 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "NKG7", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p55 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "GNLY", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p56 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "EOMES", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
##### T-reg / Proliferating T-reg 

The T-regs and proliferating T-regs can be found in clusters 3 & 7, respectively. 

```{r}
p57 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "TIGIT", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p58 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "TOP2A", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
Lastly, we find a group of mast cells in cluster 6 using CPA3. 

```{r}
p59 <- FeaturePlot(nkt_tumor, reduction = "tsne", features = "CPA3", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
Now we can add celltype identities to the object's metadata 

```{r}
nkt_tumor@meta.data %<>% mutate(label = case_when(seurat_clusters == 0 ~ "CD4+ T", 
                                                  seurat_clusters == 1 ~ "CD4+ T", 
                                                  seurat_clusters == 2 ~ "CD8+ T", 
                                                  seurat_clusters == 3 ~ "T-reg", 
                                                  seurat_clusters == 4 ~ "CD8+ T", 
                                                  seurat_clusters == 5 ~ "NK", 
                                                  seurat_clusters == 6 ~ "Mast", 
                                                  seurat_clusters == 7 ~ "Proliferating T-reg"))
Idents(nkt_tumor) <- "label"
p60 <- DimPlot(nkt_tumor, reduction = "tsne", pt.size = 1.5) + 
...
#### Recluster Adjacent Normal NK / T Cells 

```{r}
nkt_norm <- ReclusterCells(subset(pdac, subset = condition == "AdjNorm"), 
                           which.clust = c(1, 3, 7), 
                           merge.clusters = TRUE, 
                           n.HVG = 2000, 
                           n.PC = 10, 
                           k.vals = c(10, 20, 30), 
                           resolution.vals = c(.1, .2, .3, .4, .5), 
                           nn.metric = "cosine", 
                           redo.embedding = TRUE, 
                           random.seed = 629)
nkt_norm_pc <- Embeddings(nkt_norm, reduction = "pca")
```
...
# run Fit-SNE
affin_nkt_norm = PerplexityBasedNN(nkt_norm_pc, perplexity=30, random_state=629)
...
embed_nkt_norm2 = embed_nkt_norm1.optimize(n_iter=750, exaggeration=1, momentum=0.4)
```
...
nkt_norm@reductions$bh_tsne <- nkt_tumor@reductions$tsne
nkt_norm@reductions$tsne <- CreateDimReducObject(embeddings = embed_nkt_norm, 
                                                 key = "FitSNE_", 
                                                 assay = "SCT", 
                                                 global = TRUE)
p61 <- DimPlot(nkt_norm, reduction = "tsne", pt.size = 1.5) + 
       scale_color_manual(values = paletteer_d("ggsci::default_nejm")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       guides(color = guide_legend(nrow = 1, override.aes = list(size = 4)))
...
We identify the naive & memory CD4+ T cells in clusters 0, 2, & 6 using IL7R, CCR7, and CD4.

```{r}
p62 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "IL7R", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p63 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "CD4", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p64 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "CCR7", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
We can identify the CD8+ T and NK cells in clusters 3 and 1 & 4, respectively, using the following markers. 

```{r}
p65 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "CD8A", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p66 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "NKG7", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p67 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "GZMB", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p68 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "GNLY", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
FOXP3 and TOP2A highlight the T-reg and proliferating T-reg cells in clusters 5 and 7, respectively. 

```{r}
p69 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "FOXP3", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p70 <- FeaturePlot(nkt_norm, reduction = "tsne", features = "TOP2A", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
```{r}
nkt_norm@meta.data %<>% mutate(label = case_when(seurat_clusters == 0 ~ "CD4+ T", 
                                                 seurat_clusters == 1 ~ "NK", 
                                                 seurat_clusters == 2 ~ "CD4+ T", 
                                                 seurat_clusters == 3 ~ "CD8+ T", 
                                                 seurat_clusters == 4 ~ "NK", 
                                                 seurat_clusters == 5 ~ "T-reg", 
                                                 seurat_clusters == 6 ~ "CD4+ T", 
                                                 seurat_clusters == 7 ~ "Proliferating T-reg"))
Idents(nkt_norm) <- "label"
p71 <- DimPlot(nkt_norm, reduction = "tsne", pt.size = 1.5) + 
       scale_color_paletteer_d("ggsci::default_jama") + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       theme(plot.title = element_blank()) + 
...
```{r}
myo_tumor <- ReclusterCells(subset(pdac, subset = condition == "PDAC"), 
                           which.clust = c(0, 2, 6), 
                           merge.clusters = TRUE, 
                           n.HVG = 4000, 
                           n.PC = 15, 
                           k.vals = c(20, 30, 40, 50), 
                           resolution.vals = c(.2, .3, .4, .5), 
                           nn.metric = "cosine", 
                           redo.embedding = TRUE, 
                           random.seed = 629)
myo_tumor_pc <- Embeddings(myo_tumor, reduction = "pca")
```

We'll run Fit-SNE on the reclustered cells. 

```{python}
# import data
myo_tumor_pc = r.myo_tumor_pc
# run Fit-SNE
affin_myo_tumor = PerplexityBasedNN(myo_tumor_pc, perplexity=30, random_state=629)
init = initialization.pca(myo_tumor_pc, random_state=629)
tsne_myo_tumor = TSNEEmbedding(init, affin_myo_tumor, negative_gradient_method='fft')
embed_myo_tumor1 = tsne_myo_tumor.optimize(n_iter=250, exaggeration=12, momentum=0.6)
embed_myo_tumor2 = embed_myo_tumor1.optimize(n_iter=750, exaggeration=1, momentum=0.4)
```

We pull the results into R, making sure to save the Barnes-Hut t-SNE results in another reduction slot.

```{r}
embed_myo_tumor <- as.matrix(py$embed_myo_tumor2)
rownames(embed_myo_tumor) <- colnames(myo_tumor)
myo_tumor@reductions$bh_tsne <- myo_tumor@reductions$tsne
myo_tumor@reductions$tsne <- CreateDimReducObject(embeddings = embed_myo_tumor, 
                                                  key = "FitSNE_", 
                                                  assay = "SCT", 
                                                  global = TRUE)
p72 <- DimPlot(myo_tumor, reduction = "tsne", pt.size = 1.5) + 
       scale_color_manual(values = paletteer_d("ggsci::default_nejm")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       guides(color = guide_legend(nrow = 1, override.aes = list(size = 4)))
...
##### Resident & Alternatively-activated Macrophages

The resident macrophages can be found in clusters 0 & 7, and the alternatively-activated ones in cluster 2. 

```{r}
p73 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "APOE", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p74 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "C1QA", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p75 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "CD68", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
##### Classical & CD16+ Monocyte 

High expression of LYZ and present (but lower than that found in resident macrophages) expression of CD14 reveals the classical monocytes in cluster 1. FCGR3A (aka CD16) shows the CD16+ monocytes in cluster 6. 

```{r}
p76 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "LYZ", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p77 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "CD14", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p78 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "FCGR3A", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
High expression of S100A8 & S100A9 identify the neutrophils in cluster 3. 

```{r}
p79 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "S100A8", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p80 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "S100A9", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
We can identify dendritic cells in clusters 4 & 5 using the canonical markers FCER1A and CLEC10A. 

```{r}
p81 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "FCER1A", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p82 <- FeaturePlot(myo_tumor, reduction = "tsne", features = "CLEC10A", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
We can now add celltype labels to each of the clusters. 

```{r}
myo_tumor@meta.data %<>% mutate(label = case_when(seurat_clusters == 0 ~ "Resident Macrophage", 
                                                  seurat_clusters == 1 ~ "Classical Monocyte", 
                                                  seurat_clusters == 2 ~ "Alternatively-activated Monocyte", 
                                                  seurat_clusters == 3 ~ "Neutrophil", 
                                                  seurat_clusters == 4 ~ "DC", 
                                                  seurat_clusters == 5 ~ "DC", 
                                                  seurat_clusters == 6 ~ "Non-classical Monocyte", 
                                                  seurat_clusters == 7 ~ "Resident Macrophage"))
Idents(myo_tumor) <- "label"
p83 <- DimPlot(myo_tumor, reduction = "tsne", pt.size = 1.5) + 
       scale_color_paletteer_d("ggsci::default_jama") + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       theme(plot.title = element_blank()) + 
...
```{r}
myo_norm <- ReclusterCells(subset(pdac, subset = condition == "AdjNorm"), 
                           which.clust = c(0, 2, 6), 
                           merge.clusters = TRUE, 
                           n.HVG = 4000, 
                           n.PC = 10, 
...
# run Fit-SNE
affin_myo_norm = PerplexityBasedNN(myo_norm_pc, perplexity=30, random_state=629)
...
embed_myo_norm1 = tsne_myo_norm.optimize(n_iter=250, exaggeration=12, momentum=0.6)
embed_myo_norm2 = embed_myo_norm1.optimize(n_iter=750, exaggeration=1, momentum=0.4)
```

We pull the results into R, making sure to save the Barnes-Hut t-SNE results in another reduction slot.

```{r}
embed_myo_norm <- as.matrix(py$embed_myo_norm2)
rownames(embed_myo_norm) <- colnames(myo_norm)
myo_norm@reductions$bh_tsne <- myo_norm@reductions$tsne
myo_norm@reductions$tsne <- CreateDimReducObject(embeddings = embed_myo_norm, 
                                                 key = "FitSNE_", 
                                                 assay = "SCT", 
                                                 global = TRUE)
p84 <- DimPlot(myo_norm, reduction = "tsne", pt.size = 1.5) + 
       scale_color_manual(values = paletteer_d("ggsci::default_nejm")) + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       guides(color = guide_legend(nrow = 1, override.aes = list(size = 4)))
...
We can identify resident macrophages in cluster 0 using C1QA and APOE. 

```{r}
p85 <- FeaturePlot(myo_norm, reduction = "tsne", features = "C1QA", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p86 <- FeaturePlot(myo_norm, reduction = "tsne", features = "APOE", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
The DCs in cluster 2 can be found through their expression of FCER1A and CLEC10A. 

```{r}
p87 <- FeaturePlot(myo_norm, reduction = "tsne", features = "CLEC10A", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p88 <- FeaturePlot(myo_norm, reduction = "tsne", features = "FCER1A", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
Next, high S100A8 and S100A9 show us the neutrophils in cluster 3. 

```{r}
p89 <- FeaturePlot(myo_norm, reduction = "tsne", features = "S100A8", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p90 <- FeaturePlot(myo_norm, reduction = "tsne", features = "S100A9", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
Lastly, joint expression of CD14, LYZ, and FCGR3A (aka CD16) reveal the classical monocytes in clusters 1 & 4. 

```{r}
p91 <- FeaturePlot(myo_norm, reduction = "tsne", features = "CD14", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p92 <- FeaturePlot(myo_norm, reduction = "tsne", features = "FCGR3A", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
p93 <- FeaturePlot(myo_norm, reduction = "tsne", features = "LYZ", pt.size = 1.5) + 
       scale_color_gradientn(colors = wesanderson::wes_palette("Zissou1")) + 
       theme_yehlab() + 
       NoLegend() + 
       theme(axis.title = element_blank())
...
```{r}
myo_norm@meta.data %<>% mutate(label = case_when(seurat_clusters == 0 ~ "Resident Macrophage", 
                                                 seurat_clusters == 1 ~ "Classical Monocyte", 
                                                 seurat_clusters == 2 ~ "DC", 
                                                 seurat_clusters == 3 ~ "Neutrophil", 
                                                 seurat_clusters == 4 ~ "Classical Monocyte"))
Idents(myo_norm) <- "label"
p94 <- DimPlot(myo_norm, reduction = "tsne", pt.size = 1.5) + 
       scale_color_paletteer_d("ggsci::default_jama") + 
       labs(x = "Fit-SNE 1", y = "Fit-SNE 2") + 
       theme_yehlab() + 
       theme(plot.title = element_blank()) + 
...
lipid_proc_cells <- rownames(ductal@meta.data)[ductal@meta.data$label == "Lipid Proc."]
secretory_cells <- rownames(ductal@meta.data)[ductal@meta.data$label == "Secretory"]
acinar_cells <- rownames(ductal@meta.data)[ductal@meta.data$label == "Acinar"]
# fibro 
...
# NK / T
cd4t_cells <- c(rownames(nkt_tumor@meta.data)[nkt_tumor@meta.data$label == "CD4+ T"], 
                rownames(nkt_norm@meta.data)[nkt_norm@meta.data$label == "CD4+ T"])
cd8t_cells <- c(rownames(nkt_tumor@meta.data)[nkt_tumor@meta.data$label == "CD8+ T"], 
                rownames(nkt_norm@meta.data)[nkt_norm@meta.data$label == "CD8+ T"])
nk_cells <- c(rownames(nkt_tumor@meta.data)[nkt_tumor@meta.data$label == "NK"], 
                rownames(nkt_norm@meta.data)[nkt_norm@meta.data$label == "NK"])
treg_cells <- c(rownames(nkt_tumor@meta.data)[nkt_tumor@meta.data$label == "T-reg"], 
                rownames(nkt_norm@meta.data)[nkt_norm@meta.data$label == "T-reg"])
prolif_treg_cells <- c(rownames(nkt_tumor@meta.data)[nkt_tumor@meta.data$label == "Proliferating T-reg"], 
                       rownames(nkt_norm@meta.data)[nkt_norm@meta.data$label == "Proliferating T-reg"])
mast_cells <- rownames(nkt_tumor@meta.data)[nkt_tumor@meta.data$label == "Mast"]
# myeloid 
res_macro_cells <- c(rownames(myo_tumor@meta.data)[myo_tumor@meta.data$label == "Resident Macrophage"], 
                     rownames(myo_norm@meta.data)[myo_norm@meta.data$label == "Resident Macrophage"])
classic_mono_cells <- c(rownames(myo_tumor@meta.data)[myo_tumor@meta.data$label == "Classical Monocyte"], 
                        rownames(myo_norm@meta.data)[myo_norm@meta.data$label == "Classical Monocyte"])
non_classic_mono_cells <- rownames(myo_tumor@meta.data)[myo_tumor@meta.data$label == "Non-classical Monocyte"]
alt_macro_cells <- rownames(myo_tumor@meta.data)[myo_tumor@meta.data$label == "Alternatively-activated Macrophage"]
neutrophil_cells <- c(rownames(myo_tumor@meta.data)[myo_tumor@meta.data$label == "Neutrophil"], 
                      rownames(myo_norm@meta.data)[myo_norm@meta.data$label == "Neutrophil"])
dc_cells <- c(rownames(myo_tumor@meta.data)[myo_tumor@meta.data$label == "DC"], 
              rownames(myo_norm@meta.data)[myo_norm@meta.data$label == "DC"])
pdac@meta.data %<>% mutate(label_broad = case_when(seurat_clusters == 0 ~ "Macrophage", 
                           ...
                           # used previously to generate ref. pop. for filtering for CAFs / PDAC cells
                           label_fine_old = case_when(rownames(.) %in% basal_cells ~ "Basal-like PDAC", 
                           ...
                           label_fine = case_when(rownames(.) %in% basal_cells ~ "Basal-like PDAC", 
                                                    rownames(.) %in% classical1_cells ~ "Classical PDAC 1", 
                                                    rownames(.) %in% classical2_cells ~ "Classical PDAC 2", 
                                                    rownames(.) %in% lipid_proc_cells ~ "Lipid Proc.", 
                                                    rownames(.) %in% secretory_cells ~ "Secretory", 
                                                    rownames(.) %in% acinar_cells ~ "Acinar", 
                                                    rownames(.) %in% icaf_cells ~ "iCAF", 
                                                    rownames(.) %in% mycaf_cells ~ "myCAF", 
                                                    rownames(.) %in% apcaf_cells ~ "apCAF", 
                                                    rownames(.) %in% cd4t_cells ~ "CD4+ T",
                                                    rownames(.) %in% cd8t_cells ~ "CD8+ T", 
                                                    rownames(.) %in% nk_cells ~ "NK", 
                                                    rownames(.) %in% treg_cells ~ "T-reg", 
                                                    rownames(.) %in% prolif_treg_cells ~ "Proliferating T-reg", 
                                                    rownames(.) %in% mast_cells ~ "Mast", 
                                                    rownames(.) %in% res_macro_cells ~ "Resident Macrophage",
                                                    rownames(.) %in% classic_mono_cells ~ "Classical Monocyte",
                                                    rownames(.) %in% non_classic_mono_cells ~ "Non-classical Monocyte",
                                                    rownames(.) %in% alt_macro_cells ~ "Alternatively-activated Macrophage",
                                                    rownames(.) %in% neutrophil_cells ~ "Neutrophil",
                                                    rownames(.) %in% dc_cells ~ "DC", 
                                                    TRUE ~ label_broad))
...
FindFilteredMarkers <- function(obj.1 = NULL,                 # the object containing cells we're interested in 
                                obj.2 = NULL,                 # the reference object to filter against
  ...
  Idents(obj.1) <- ident.1
  Idents(obj.2) <- ident.2
  DefaultAssay(obj.1) <- "SCT"
  DefaultAssay(obj.2) <- "SCT"
  markers <- FindAllMarkers(obj.1, 
                            logfc.threshold = logfc.cutoff, 
                            test.use = test.use, 
                            only.pos = TRUE, 
                            verbose = FALSE, 
                            random.seed = 312) %>% 
             filter(p_val_adj < pval.cutoff)
  ...
  gene_means_by_clust <- t(obj.2@assays$SCT@data) %>% 
                         as.data.frame() %>% 
                         mutate(label_fine = obj.2[[ident.2]][, 1]) %>% 
                         group_by(label_fine) %>% 
  ...
  for (i in loop_celltypes) {
    top_exp_genes <- gene_means_by_clust %>% 
                     ...
                     t() %>% 
                     as.data.frame() %>% 
                     filter(V1 > quantile(V1, quantile.cutoff)) %>% 
                     mutate(gene = rownames(.)) %>% 
                     pull(gene)
    high_exp_genes <- c(high_exp_genes, top_exp_genes)
  }
  high_exp_genes <- unique(high_exp_genes)
  markers %<>% filter(!gene %in% high_exp_genes)
  return(markers)
}

all_markers <- FindSpecificMarkers(pdac, ident.use = "label_broad", 
                                   de.method = "wilcox", 
                                   perc.cutoff = 0.9, 
                                   log2fc.cutoff = 0.1, 
                                   fdr.cutoff = 0.05)
gc()
caf_markers_test <- FindFilteredMarkers(obj.1 = caf, 
                                        obj.2 = pdac, 
                                        ident.1 = "label", 
                                        ident.2 = "label_fine", 
                                        test.use = "wilcox",
                                        ...
                                        extra.cell.filter = NULL)
gc()
pdac_markers_test <- FindFilteredMarkers(obj.1 = pdac_ductal, 
                                         obj.2 = pdac, 
                                         ident.1 = "label", 
                                         ident.2 = "label_fine", 
                                         test.use = "wilcox",
                                         ...
                                         extra.cell.filter = NULL)
gc()
pdac_broad_markers_test <- FindFilteredMarkers(obj.1 = pdac_ductal, 
                                               obj.2 = pdac, 
                                               ident.1 = "label_broad", 
                                               ident.2 = "label_fine", 
                                               test.use = "wilcox",
                                               ...
                                               extra.cell.filter = NULL)
gc()
ductal_markers_test <- FindFilteredMarkers(obj.1 = ductal, 
                                           obj.2 = pdac, 
                                           ident.1 = "label", 
                                           ident.2 = "label_fine", 
                                           test.use = "wilcox",
                                           ...
                                           extra.cell.filter = NULL)
gc()
nkt_tumor_markers_test <- FindFilteredMarkers(obj.1 = nkt_tumor, 
                                              obj.2 = pdac, 
                                              ident.1 = "label", 
                                              ident.2 = "label_fine", 
                                              test.use = "wilcox",
                                              ...
                                              logfc.cutoff = 0.25, 
                                              quantile.cutoff = 0.9, 
                                              extra.cell.filter = c(cd4t_cells, cd8t_cells, nk_cells, mast_cells, treg_cells, prolif_treg_cells))
gc()
nkt_norm_markers_test <- FindFilteredMarkers(obj.1 = nkt_norm, 
                                             obj.2 = pdac, 
                                             ident.1 = "label", 
                                             ident.2 = "label_fine", 
                                             test.use = "wilcox",
                                             ...
                                             extra.cell.filter = c(cd4t_cells, cd8t_cells, nk_cells, mast_cells, treg_cells, prolif_treg_cells))
gc()
myo_tumor_markers_test <- FindFilteredMarkers(obj.1 = myo_tumor, 
                                              obj.2 = pdac, 
                                              ident.1 = "label", 
                                              ident.2 = "label_fine", 
                                              test.use = "wilcox",
                                              ...
                                              logfc.cutoff = 0.25, 
                                              quantile.cutoff = 0.9, 
                                              extra.cell.filter = c(res_macro_cells, alt_macro_cells, classic_mono_cells, non_classic_mono_cells, neutrophil_cells, dc_cells))
gc()
myo_norm_markers_test <- FindFilteredMarkers(obj.1 = myo_norm, 
                                             obj.2 = pdac, 
                                             ident.1 = "label", 
                                             ident.2 = "label_fine", 
                                             test.use = "wilcox",
                                             ...
                                             extra.cell.filter = c(res_macro_cells, alt_macro_cells, classic_mono_cells, non_classic_mono_cells, neutrophil_cells, dc_cells))
gc()
fibro_markers_test <- FindFilteredMarkers(obj.1 = fibro, 
                                          obj.2 = pdac, 
                                          ident.1 = "label", 
                                          ident.2 = "label_fine", 
                                          test.use = "wilcox",
...
openxlsx::addWorksheet(wb, sheetName = "Broad_Celltypes")
openxlsx::writeData(wb, sheet = "Broad_Celltypes", x = all_markers)
openxlsx::addWorksheet(wb, sheetName = "PDAC_Broad")
openxlsx::writeData(wb, sheet = "PDAC_Broad", x = pdac_broad_markers_test)
openxlsx::addWorksheet(wb, sheetName = "PDAC_Fine")
openxlsx::writeData(wb, sheet = "PDAC_Fine", x = pdac_markers_test)
openxlsx::addWorksheet(wb, sheetName = "Ductal")
openxlsx::writeData(wb, sheet = "Ductal", x = (ductal_markers_test %>% filter(cluster %in% c("Acinar", "Lipid Proc.", "Secretory"))))
openxlsx::addWorksheet(wb, sheetName = "CAF")
openxlsx::writeData(wb, sheet = "CAF", x = caf_markers_test)
openxlsx::addWorksheet(wb, sheetName = "Stroma")
openxlsx::writeData(wb, sheet = "Stroma", x = (fibro_markers_test %>% filter(cluster %in% c("Endothelial", "Perivascular"))))
openxlsx::addWorksheet(wb, sheetName = "NKT_Tumor")
openxlsx::writeData(wb, sheet = "NKT_Tumor", x = nkt_tumor_markers_test)
openxlsx::addWorksheet(wb, sheetName = "NKT_Normal")
openxlsx::writeData(wb, sheet = "NKT_Normal", x = nkt_norm_markers_test)
openxlsx::addWorksheet(wb, sheetName = "Myeloid_Tumor")
openxlsx::writeData(wb, sheet = "Myeloid_Tumor", x = myo_tumor_markers_test)
openxlsx::addWorksheet(wb, sheetName = "Myeloid_Normal")
openxlsx::writeData(wb, sheet = "Myeloid_Normal", x = myo_norm_markers_test)
openxlsx::saveWorkbook(wb, file = "../Data/Gene_Sets/Elyada_Subcluster_Markers_All.xlsx")
...
saveRDS(nkt_tumor, file = "~/Desktop/Data/Tuveson_Seurat_Objs/Elyada_NKT_tumor.Rds")
saveRDS(nkt_norm, file = "~/Desktop/Data/Tuveson_Seurat_Objs/Elyada_NKT_normal.Rds")
saveRDS(myo_tumor, file = "~/Desktop/Data/Tuveson_Seurat_Objs/Elyada_myeloid_tumor.Rds")
saveRDS(myo_norm, file = "~/Desktop/Data/Tuveson_Seurat_Objs/Elyada_myeloid_normal.Rds")
```
